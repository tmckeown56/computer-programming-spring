...
# removed the get_glasses function and replaced it with the new equation using temperature, chance of precipitation, and percentage of street work rather than the day.
	glasses = math.floor((temperature - 10)*(1 - chance_of_precipitation)*(1 - street_work))
	lemonade_cost = glasses * cost
	assets -= lemonade_cost
# removed the get price function and replaced it with a new equation that adds the leading number of temperature to the cost that is inputted
	price = cost + math.floor(temperature / 10)
# removed the get_signs function and replaced it with an equation that calculates the number of signs based on the number of glasses
	signs = math.floor(glasses / 15)
